...
$clientId = PAYPAL_CLIENT_ID ?: getenv('PAYPAL_CLIENT_ID') ?: '';
if (!$clientId) {
    http_response_code(500);
    echo '<h3>Configuration error: Missing PAYPAL_CLIENT_ID</h3>';
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        body { font-family: 'Inter', sans-serif; color: white; height: 100vh; overflow: hidden;
            background: linear-gradient(234deg, #5a7188, #0d3053, #3971a8);
            background-size: 600% 600%; animation: gradient-shift 22s ease infinite; }
        ...
        .logo-svg { width: 60px; height: 60px; stroke: white; stroke-width: 2; fill: none; }
        .tracking-wide { letter-spacing: 4px; }
        @keyframes gradient-shift { 0% { background-position: 2% 0%; } 50% { background-position: 99% 100%; } 100% { background-position: 2% 0%; } }
    </style>
    <script src="https://www.paypal.com/sdk/js?client-id=<?php echo htmlspecialchars($clientId, ENT_QUOTES); ?>&currency=PHP"></script>
</head>
<body>
    <div class="container d-flex justify-content-center align-items-center vh-100">
        <div class="card glass-card shadow-lg" style="width: 100%; max-width: 380px;">
            <div class="card-body p-5 text-center">
                <div class="mb-4">
                    <!-- Prefer images/logo.png; fallback to /Logo.png if present -->
                    <img src="images/logo.png" alt="Avera Logo" class="logo-svg" onerror="this.onerror=null; this.src='Logo.png';">
                </div>
                ...
                <div class="small text-white-50 mt-3">256-bit SSL Secure Payment</div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        paypal.Buttons({
            ...
            createOrder: function(data, actions) {
                return fetch('../src/paypal/create-order.php', {
                    method: 'post',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ amount: '850.00', currency: 'PHP' })
                })
                .then(res => res.json())
                .then(orderData => orderData.data?.id || orderData.id);
            },
                    ...
                    .then(orderData => {
                        ...
                        cardBody.innerHTML = `
                            ...
                            <div class="text-start">
                                ...
                                <p class="font-monospace text-white mb-0" style="font-size: 0.9rem; letter-spacing: 1px;">${orderData.transaction_id || orderData.id || data.orderID}</p>
                            </div>
                        `;
                    });
            }
